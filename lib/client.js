...
function createSettingsSection(ctx) {
  return function OmpAdvisorSettingsSection() {
    ...
    const write = useCallback((field, next) => {
      setWriteError(null);
      void ctx.connection.rpc.call("/dsh-omp-advisor", "update", { patch: { [field]: next } }).then((result) => {
        const value2 = unwrapRpcResult(result, "advisor settings update");
        setView((current) => current ? { ...current, settings: value2.settings } : current);
      }).catch((err) => {
