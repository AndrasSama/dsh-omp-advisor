...
function createSettingsSection(ctx) {
  return function OmpAdvisorSettingsSection() {
    ...
    const memoryApprove = (writeId) => {
      void ctx.connection.rpc.call("/dsh-omp-advisor", "memoryApprove", { writeId }).then(() => memoryRescan()).catch((err) => setWriteError(String(err instanceof Error ? err.message : err)));
    };
    const memoryDiscard = (writeId) => {
      void ctx.connection.rpc.call("/dsh-omp-advisor", "memoryDiscard", { writeId }).then(() => memoryRescan()).catch((err) => setWriteError(String(err instanceof Error ? err.message : err)));
    };
    const resumeAdvisor = (sessionId, advisorName) => {
      void ctx.connection.rpc.call("/dsh-omp-advisor", "resume", { sessionId, advisor: advisorName }).then((result) => {
        const value2 = unwrapRpcResult(result, "advisor resume");
        if (value2?.ok === false) {
          ...
          (current) => current ? {
            ...
            sessions: (current.sessions ?? []).map(
              (session) => session.sessionId === sessionId ? {
                ...session,
                advisors: session.advisors.map(
                  (advisor) => advisor.name === advisorName ? { ...advisor, status: "running", haltReason: void 0, lastError: void 0 } : advisor
                )
              } : session
            )
          } : current
        );
      }).catch((err) => setWriteError(String(err instanceof Error ? err.message : err)));
    };
    return /* @__PURE__ */ React.createElement("div", { style: styles.root }, writeError && /* @__PURE__ */ React.createElement("div", { style: styles.hint }, "Settings write failed: ", writeError), /* @__PURE__ */ React.createElement("div", { style: styles.tabBar }, tabs.map((item) => /* @__PURE__ */ React.createElement(
    ...
    ), advisor.name, " \xB7 ", advisor.status, advisor.backlog > 0 ? ` \xB7 backlog ${advisor.backlog}` : "", ` \xB7 ${advisor.reviewsCompleted} reviews / ${advisor.adviceDelivered} notes`, (advisor.status === "halted" || advisor.status === "error" || advisor.status === "quota_exhausted") && /* @__PURE__ */ React.createElement(
      "button",
      {
      ...
      "Resume"
    )))), session.advisors.filter((advisor) => advisor.status === "halted" && advisor.haltReason === "context-overflow").map((advisor) => /* @__PURE__ */ React.createElement("span", { key: `${advisor.name}-ctxhint`, style: { ...styles.hint, color: "#e0b050", whiteSpace: "pre-wrap" } }, "\u24D8 ", advisor.name, ": its conversation outgrew the model's context window. Assign it a model with a larger context window in the Advisors tab, then click Resume.")), session.advisors.filter((advisor) => advisor.lastError).map((advisor) => /* @__PURE__ */ React.createElement(
      "span",
      {
...
function splitAdvisorsByWorkspace(advisors, cwd) {
  const active = [];
  const inactive = [];
...
function buildWorkspaceAdvisor(options) {
  const { name: name2, provider, model, cwd, preset } = options;
  return {
    name: name2,
    provider,
    model,
    ...
    ...preset ? { instructions: preset.soul, skills: [...preset.skills], preset: preset.id } : {}
  };
}

// src/client/sidebar.tsx
var { useEffect: useEffect2, useRef: useRef2, useState: useState2 } = React2;
...
async function setAdvisorWorkspaceRpc(advisor, cwd, active) {
  const connection = connectionRef;
  ...
  unwrapRpcResult(result, "advisor workspace toggle");
}
async function addWorkspaceAdvisorRpc(entry) {
  const connection = connectionRef;
  if (!connection) throw new Error("no connection");
  const result = await connection.rpc.call("/dsh-omp-advisor", "addWorkspaceAdvisor", { entry });
  unwrapRpcResult(result, "add workspace advisor");
}
function loadModelCatalog() {
  ...
  return fetchModelCatalog(connection).catch(
    () => null
  );
}
var STATUS_COLORS2 = {
...
function presetMenuItem(dark, hovered) {
  return {
    textAlign: "left",
    border: "none",
    borderRadius: 7,
...
function AdvisorsMonitorTab(props) {
  ...
  const [presetMenuOpen, setPresetMenuOpen] = useState2(false);
  const [hoveredPreset, setHoveredPreset] = useState2(null);
  const presetMenuRef = useRef2(null);
  ...
  const workspaceCwd = scoped ? sessions.find((session) => session.sessionId === scoped)?.cwd : void 0;
  const advisorsList = configured;
  const workspaceSplit = workspaceCwd ? splitAdvisorsByWorkspace(advisorsList, workspaceCwd) : null;
  const runToggle = (promise, label) => {
    setActionError(null);
    void promise.then(() => pollOnce()).catch((err) => setActionError(`${label}: ${String(err instanceof Error ? err.message : err)}`));
  };
  const enableHere = (name2) => {
    if (!workspaceCwd || !name2) return;
    runToggle(setAdvisorWorkspaceRpc(name2, workspaceCwd, true), `enable "${name2}"`);
  };
  const disableHere = (name2) => {
    if (!workspaceCwd || !name2) return;
    runToggle(setAdvisorWorkspaceRpc(name2, workspaceCwd, false), `disable "${name2}"`);
  };
  ...
  const renderSessionCard = (session) => {
    ...
    const matching = session.cwd ? configured.filter(
  ...
  return /* @__PURE__ */ React2.createElement("div", { style: panel }, scoped && !scopedSession && /* @__PURE__ */ React2.createElement("div", { style: cardStyle }, /* @__PURE__ */ React2.createElement("strong", null, "No advisors in this workspace"), /* @__PURE__ */ React2.createElement("span", { style: hint }, "No advisor data for this session yet \u2014 the master switch is off (Settings \u2192 Ward Council \u2192 General), or no configured advisor's workspace patterns match this session's workspace.")), !scoped && sessions.length === 0 && /* @__PURE__ */ React2.createElement("div", { style: cardStyle }, /* @__PURE__ */ React2.createElement("strong", null, "No advisor sessions"), /* @__PURE__ */ React2.createElement("span", { style: hint }, "Advisors attach to sessions when the plugin is enabled (Settings \u2192 Ward Council \u2192 General) and a session matches an advisor's workspace patterns.")), scopedSession && renderSessionCard(scopedSession), scoped && workspaceCwd && workspaceSplit && /* @__PURE__ */ React2.createElement("div", { style: cardStyle }, /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" } }, /* @__PURE__ */ React2.createElement("strong", null, "Advisors \u2014 this workspace"), /* @__PURE__ */ React2.createElement("span", { style: chip, title: workspaceCwd }, basename(workspaceCwd))), actionError && /* @__PURE__ */ React2.createElement("span", { style: { ...hint, color: "#dc7070" } }, actionError), advisorsList.length === 0 ? /* @__PURE__ */ React2.createElement("span", { style: hint }, "No advisors configured yet \u2014 add one below.") : /* @__PURE__ */ React2.createElement(React2.Fragment, null, workspaceSplit.active.map((entry, index) => {
      ...
      /* @__PURE__ */ React2.createElement(
        "span",
        {
          ...
          style: {
            ...
            height: 8,
            borderRadius: "50%",
            background: live ? STATUS_COLORS2[live.status] ?? "#8a8a8a" : "#4caf7d",
            display: "inline-block"
          }
        }
      ),
      /* @__PURE__ */ React2.createElement("span", { style: { fontWeight: 600 } }, entry.name || "unnamed"),
      /* @__PURE__ */ React2.createElement("span", { style: hint }, live ? live.status : "active here"),
      /* @__PURE__ */ React2.createElement("button", { style: actionButton, onClick: () => disableHere(entry.name ?? "") }, "Disable here")
    );
  }), workspaceSplit.inactive.map(({ entry, reason }, index) => /* @__PURE__ */ React2.createElement(
    "div",
    {
    ...
    /* @__PURE__ */ React2.createElement("span", { style: { fontWeight: 600 } }, entry.name || "unnamed"),
    /* @__PURE__ */ React2.createElement("span", { style: chip }, reason === "off" ? "off" : reason === "disabled-here" ? "disabled here" : "not in this workspace"),
    /* @__PURE__ */ React2.createElement("button", { style: actionButton, onClick: () => enableHere(entry.name ?? "") }, "Enable here")
  ))), /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginTop: 4 } }, /* @__PURE__ */ React2.createElement("div", { ref: presetMenuRef, style: { position: "relative", display: "inline-block" } }, /* @__PURE__ */ React2.createElement(
    "button",
    {
      style: actionButton,
      "aria-haspopup": "menu",
    ...
    "Add advisor \u25BE"
  ), presetMenuOpen && /* @__PURE__ */ React2.createElement("div", { style: presetMenuPanel(dark), role: "menu" }, ADVISOR_PRESETS.map((preset) => /* @__PURE__ */ React2.createElement(
    "button",
    {
      key: preset.id,
      role: "menuitem",
