...
// src/advisor-workspace.ts
function workspacePatternMatches(pattern, cwd) {
  const trimmed = pattern.trim();
  if (trimmed === "") return false;
  if (!cwd) return false;
  return trimmed.startsWith("=") ? cwd === trimmed.slice(1).trim() : cwd.includes(trimmed);
}
function advisorMatchesWorkspacePatterns(workspaces, cwd) {
...
function disableAdvisorHere(advisors, name2, cwd) {
  if (!cwd) return advisors;
  return advisors.map((entry) => {
    if (entry.name !== name2) return entry;
...
function uniqueAdvisorName(base, existing) {
...
// src/memory/engines.ts
import { existsSync, readdirSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
var BUILTIN_MD_ENGINE = "builtin-md";
var MEMORY_PRESET_VERSION = 2;
var PRESET_ENGINES = [
  {
...
function coerceWriteGate(raw) {
  return raw === "auto" || raw === "readonly" ? raw : "approval";
}
function coerceEngine(raw) {
  if (!raw || typeof raw !== "object") return void 0;
  const e = raw;
  if (typeof e.id !== "string" || !e.id.trim()) return void 0;
  ...
  return {
    ...
    ...Array.isArray(e.args) ? { args: e.args.filter((a) => typeof a === "string") } : {},
    ...typeof e.cwd === "string" && e.cwd.trim() ? { cwd: e.cwd.trim() } : {},
    ...Object.keys(env).length > 0 ? { env } : {},
    ...typeof e.url === "string" && e.url.trim() ? { url: e.url.trim() } : {},
    ...typeof e.resolveScript === "string" && e.resolveScript.trim() ? { resolveScript: e.resolveScript.trim() } : {},
    ...typeof tools.recall === "string" || typeof tools.store === "string" || typeof tools.forget === "string" || typeof tools.health === "string" ? {
      tools: {
...
function normalizeMemorySettings(raw) {
  ...
  const persistedVersion = Number.isFinite(value.presetVersion) ? value.presetVersion : 0;
  ...
  const perEngineRaw = Number.isFinite(value.recallMaxPerEngine) ? value.recallMaxPerEngine : 3;
  const budgetRaw = Number.isFinite(value.recallBudgetChars) ? value.recallBudgetChars : 6e3;
  return {
    enabled: value.enabled !== false,
    writeGate: coerceWriteGate(value.writeGate),
    engines: [...byId.values()],
    recallMaxPerEngine: Math.min(10, Math.max(1, Math.round(perEngineRaw))),
    recallBudgetChars: Math.min(4e4, Math.max(500, Math.round(budgetRaw))),
...
function resolvePackageScript(specifier) {
...
function resolveEngineSpawn(engine) {
...
// src/memory/md-store.ts
import { mkdir, readFile, writeFile } from "node:fs/promises";
import { join as join2 } from "node:path";
var MEMORY_DIR_NAME = ".dsh-omp-advisor";
var LESSONS_FILE_NAME = "lessons.md";
function lessonsPath(cwd) {
  return join2(cwd, MEMORY_DIR_NAME, LESSONS_FILE_NAME);
}
function renderLessonEntry(lesson, time = /* @__PURE__ */ new Date()) {
...
async function appendLesson(cwd, lesson) {
  ...
  await mkdir(join2(cwd, MEMORY_DIR_NAME), { recursive: true });
...
function recallLessons(entries, query, limit) {
  ...
  const scored = docs.map((doc) => {
    ...
    for (const term of terms) {
      ...
      if (doc.tagSet.has(term)) score += 0.5;
    }
    return { doc, score };
  });
  return scored.filter((item) => item.score > 0).sort((a, b) => b.score - a.score || b.doc.index - a.doc.index).slice(0, limit).map((item) => ({
    engineId: "builtin-md",
    id: `lesson-${item.doc.index + 1}`,
    score: Math.round(item.score * 1e3) / 1e3,
    text: item.doc.entry.text.trim()
  }));
}
async function recallFromWorkspace(cwd, query, limit) {
  try {
...
var MCP_PROBE_TIMEOUT_MS = 8e3;
var MCP_CALL_TIMEOUT_MS = 1e4;
var MCP_IDLE_TTL_MS = 5 * 6e4;
var MCP_PROTOCOL_VERSION = "2024-11-05";
var cache = /* @__PURE__ */ new Map();
function cacheKey(engine) {
...
function extractMcpText(result) {
  ...
  const text = content.filter((block) => !!block && block.type === "text").map((block) => typeof block.text === "string" ? block.text : "").join("\n");
  if (payload.isError === true) throw new Error(text || "MCP tool returned an error");
  return text;
}
function connectStdio(engine) {
  return new Promise((resolve2, reject) => {
    if (!engine.command) {
...
`);
    ...
    void (async () => {
      try {
        ...
        const listed = await request("tools/list", {}, MCP_PROBE_TIMEOUT_MS);
        const toolNames = (listed?.tools ?? []).map((tool) => tool?.name).filter((name2) => typeof name2 === "string");
        if (settled) return;
        settled = true;
        resolve2({
          engineId: engine.id,
          toolNames,
          async call(tool, args2, timeoutMs = MCP_CALL_TIMEOUT_MS) {
            const result = await request("tools/call", { name: tool, arguments: args2 }, timeoutMs);
...
async function httpRequest(url, message, timeoutMs, envHeaders) {
...
async function connectHttp(engine) {
  ...
  const request = async (method, params, timeoutMs) => {
    ...
    const response = await httpRequest(engine.url, envelope, timeoutMs, headers);
...
async function getMcpSession(engine) {
  ...
  const session = engine.transport === "http" ? await connectHttp(engine) : await connectStdio(engine);
  const entry = {
    session,
    lastUsed: Date.now(),
    reaper: setTimeout(() => {
...
async function probeMcpEngine(engine) {
  const session = await getMcpSession(engine);
  return session.toolNames;
}

// src/memory/pack.ts
function packMemoryItems(items, opts) {
...
// src/memory/manager.ts
var ENGINE_RECALL_TIMEOUT_MS = 1e4;
var PENDING_FILE_NAME = "pending-memory.json";
...
function buildStoreArgs(engine, lesson) {
...
var MemoryManager = class _MemoryManager {
  ...
  async probeEngine(engine) {
    ...
    if (engine.cwd && !existsSync2(expandHome(engine.cwd))) {
      return { available: false, detail: `not installed: ${engine.cwd} missing` };
    }
    try {
      const toolNames = await probeMcpEngine(engine);
      const wanted = [engine.tools?.recall, engine.tools?.store].filter((t) => !!t);
  ...
  async probeAll() {
    ...
    const probes = await Promise.all(
      this.settings.engines.map(async (engine) => [engine.id, await this.probeEngine(engine)])
    );
  ...
  usableEngines(ids) {
    if (!this.settings.enabled) return [];
    const wanted = ids && ids.length > 0 ? new Set(ids) : void 0;
  ...
  /* -------------------------------- recall --------------------------------- */
  async recallOpenViking(engine, query, limit) {
    const service = this.host.getService("openvikingMemory");
    const client = service?.client;
    ...
    const items = [];
    for (const value of Object.values(response.result)) {
      if (!Array.isArray(value)) continue;
      for (const [index, raw] of value.entries()) {
        const item = raw ?? {};
        const text = typeof item.abstract === "string" && item.abstract ? item.abstract : typeof item.content === "string" ? item.content : "";
  ...
  static parseMcpRecallText(engineId, text, limit) {
    ...
    const list = Array.isArray(parsed) ? parsed : parsed && typeof parsed === "object" ? ["results", "items", "memories", "lessons", "matches"].map((key) => parsed[key]).find((value) => Array.isArray(value)) ?? [] : [];
    return list.slice(0, limit * 2).map((raw, index) => {
      const item = raw ?? {};
      const textValue = typeof item.text === "string" ? item.text : typeof item.content === "string" ? item.content : typeof item.summary === "string" ? item.summary : typeof item.title === "string" ? item.title : JSON.stringify(raw);
      return {
        engineId,
        id: typeof item.id === "string" ? item.id : typeof item.uri === "string" ? item.uri : typeof item.path === "string" ? item.path : `${engineId}:${index}`,
        score: typeof item.score === "number" ? item.score : 0,
        text: textValue
      };
    });
  }
  async recallMcp(engine, query, limit) {
    const tool = engine.tools?.recall;
    if (!tool) return [];
    const session = await getMcpSession(engine);
    const text = await session.call(tool, { query, limit, top: limit }, ENGINE_RECALL_TIMEOUT_MS);
    return _MemoryManager.parseMcpRecallText(engine.id, text, limit);
  }
  /**
      ...
      engines.map(async (engine) => {
        try {
          const items = await Promise.race([
            engine.kind === "builtin-md" ? recallFromWorkspace(opts.cwd, opts.query, perEngine) : engine.id === "openviking" && engine.kind === "service" ? this.recallOpenViking(engine, opts.query, perEngine) : this.recallMcp(engine, opts.query, perEngine),
            new Promise(
              (_, reject) => setTimeout(() => reject(new Error("recall timeout")), ENGINE_RECALL_TIMEOUT_MS)
            )
          ]);
          return items;
        } catch (error) {
  ...
  async writeNow(cwd, lesson, engineIds) {
    const stored = [];
    const failed = [];
  ...
  pendingPath(cwd) {
    return join3(cwd, MEMORY_DIR_NAME, PENDING_FILE_NAME);
  }
  async persistPending(cwd) {
    try {
      const forCwd = this.pending.filter((write) => write.cwd === cwd);
      await mkdir2(join3(cwd, MEMORY_DIR_NAME), { recursive: true });
      await writeFile2(this.pendingPath(cwd), JSON.stringify(forCwd, null, 2), "utf8");
    } catch (error) {
  ...
  async loadPending(cwd) {
    if (!cwd || this.loadedCwds.has(cwd)) return;
    this.loadedCwds.add(cwd);
    try {
      const raw = JSON.parse(await readFile2(this.pendingPath(cwd), "utf8"));
  ...
  async store(opts) {
    ...
    if (gate === "auto") {
      ...
      this.host.recordEvent?.("memory-write", {
        ...
        detail: `auto \xB7 ${stored.join(", ") || "none"}${failed.length ? ` \xB7 failed: ${failed.join("; ")}` : ""}`
      });
  ...
  async approve(writeId) {
    const index = this.pending.findIndex((write2) => write2.id === writeId);
    ...
    this.host.recordEvent?.("memory-write", { advisor: write.advisor, detail: `approved \xB7 ${stored.join(", ") || "none"}` });
    return { ok: true, detail: `stored: ${stored.join(", ") || "none"}${failed.length ? ` \xB7 failed: ${failed.join("; ")}` : ""}` };
  }
  /** Discard one pending write (monitor surfaces). */
  async discard(writeId) {
    const index = this.pending.findIndex((write2) => write2.id === writeId);
      ...
      try {
        ...
        switch (endpoint) {
          ...
          case "memoryDiscard": {
            ...
            const result = endpoint === "memoryApprove" ? await service.memoryApprove(writeId) : await service.memoryDiscard(writeId);
...
function freshTempIndexPath() {
  for (let attempt = 0; attempt < 5; attempt++) {
    const candidate = join4(tmpdir(), `dsh-omp-advisor-index-${randomBytes(8).toString("hex")}`);
  ...
  return join4(tmpdir(), `dsh-omp-advisor-index-${randomBytes(12).toString("hex")}-${Date.now()}`);
}
...
async function createRestorePoint(cwd, opts) {
  ...
  const run = (args) => new Promise((resolve2) => {
    ...
    let settled = false;
    const child = spawn2("git", args, { cwd, env: envIdx, stdio: ["ignore", "pipe", "pipe"] });
    const timer = setTimeout(() => {
      if (!settled) {
        settled = true;
        child.kill("SIGKILL");
...
import { resolve, relative, join as join5 } from "node:path";
...
async function loadContextFiles(cwd) {
  ...
  for (const name2 of CONTEXT_FILE_NAMES) {
    try {
      const content = await readFile4(join6(cwd, name2), "utf8");
...
var MEMORY_LESSON_RE = /<advisor-memory(?:\s+tags="([^"]*)")?\s*>([\s\S]*?)<\/advisor-memory>/;
function extractMemoryLesson(blocks) {
  for (const block of blocks) {
    ...
    if (!text) continue;
    const tags = (match[1] ?? "").split(",").map((tag) => tag.trim()).filter(Boolean).slice(0, 8);
    return { text: text.slice(0, 2e3), tags };
  }
  return void 0;
}
...
${advise_tool_default.trim()}`);
    ...
    if (this.host.memoryEnabled === true) parts.push(memory_default.trim());
...
var SessionAdvisorRuntime = class {
      ...
      while (!this.disposed && slot.queued.length > 0) {
        ...
        try {
          ...
          if (isQuotaFailure(error)) {
            ...
            this.host.recordEvent?.("quota", slot.entry.name, `cooldown ${QUOTA_COOLDOWN_MS / 1e3}s`);
            return;
          }
  ...
  emitNotes(notes) {
    ...
    for (const advisorNote of notes) {
      this.host.recordEvent?.(
        "advice",
        advisorNote.advisor,
        `${advisorNote.severity ?? "nit"} \xB7 ${steerNotes.includes(advisorNote) ? "steer" : "inject"}`
      );
    }
  }
  ...
  onTurnEnd(reason) {
    ...
    this.continueTimer = setTimeout(() => {
      ...
      this.host.recordEvent?.("continue-sent", void 0, `attempt ${attempt}/${capLabel}`);
...
var advisorEntrySchema = z.object({
  ...
  disabledWorkspaces: z.array(z.string()).default([]).description(
  ...
  memoryEngines: z.array(z.string()).default([]).description(
...
var advisorSettingsSchema = z.object({
  ...
  memory: z.any().default({}).description(
...
function advisorMatchesWorkspace(entry, cwd) {
  const disabled = (entry.disabledWorkspaces ?? []).map((pattern) => pattern.trim()).filter((pattern) => pattern !== "");
  if (disabled.some((pattern) => {
  ...
  for (const entry of advisors) {
    ...
    deduped.push({
      ...
      ...Array.isArray(entry.disabledWorkspaces) ? {
        disabledWorkspaces: entry.disabledWorkspaces.filter((w) => typeof w === "string" && w.trim() !== "").map((w) => w.trim())
      } : {},
      ...Array.isArray(entry.memoryEngines) ? {
        memoryEngines: entry.memoryEngines.filter((e) => typeof e === "string" && e.trim() !== "").map((e) => e.trim())
      } : {},
...
function normalizeSettingsLenient(raw) {
  ...
  for (const entry of advisors) {
    ...
    preserved.push({
      ...
      ...Array.isArray(e.disabledWorkspaces) ? { disabledWorkspaces: e.disabledWorkspaces.filter((w) => typeof w === "string") } : {},
      ...Array.isArray(e.memoryEngines) ? { memoryEngines: e.memoryEngines.filter((m) => typeof m === "string") } : {},
      enabled: e.enabled !== false
    });
  }
  const severities = Array.isArray(value.interruptSeverities) ? value.interruptSeverities.filter(
...
var EVENT_RING_LIMIT = 100;
var EVENT_DETAIL_LIMIT = 160;
...
var AdvisorService = class extends Service {
    ...
    this.memory = new MemoryManager(
      {
        ...
        getService: (name2) => typeof this.hostCtx.get === "function" ? this.hostCtx.get(name2) : void 0,
  ...
  recordEvent(kind, fields) {
    ...
    this.events.push({
      ...
      ...detail ? { detail: detail.length > EVENT_DETAIL_LIMIT ? `${detail.slice(0, EVENT_DETAIL_LIMIT)}\u2026` : detail } : {}
    });
   ...
   */
  setAdvisorWorkspace(advisorName, cwd, active) {
    ...
    const advisors = active ? enableAdvisorHere(current.advisors, advisorName, cwd) : disableAdvisorHere(current.advisors, advisorName, cwd);
  ...
  addWorkspaceAdvisor(rawEntry) {
    ...
    const entry = {
      ...
      ...Array.isArray(sent.workspaces) ? {
        workspaces: sent.workspaces.filter((w) => typeof w === "string" && w.trim() !== "").map((w) => w.trim())
      } : {},
      ...typeof sent.instructions === "string" && sent.instructions.trim() !== "" ? { instructions: sent.instructions } : {},
      ...Array.isArray(sent.skills) ? { skills: sent.skills.filter((s) => typeof s === "string" && s.trim() !== "") } : {},
      ...
      try {
        ...
        if (point) {
          ...
          this.recordEvent("restore-point", {
            sessionId: id,
            detail: `${label} \xB7 ${point.sha.slice(0, 7)}`
          });
        }
      } catch (err) {
    ...
    const runtime = new SessionAdvisorRuntime(
      {
        ...
        recordEvent: (kind, advisor, detail) => this.recordEvent(kind, { advisor, sessionId: id, ...detail ? { detail } : {} }),
        // Advisor memory (v0.7.0): recall rides the review, lessons ride the gate.
        recallMemory: (_advisorName, engineIds, deltaText) => this.memory.recall({ cwd, engineIds, query: deltaText.slice(-4e3) }),
  ...
  snapshot(sessionId) {
    ...
    return {
      sessionId,
      ...runtime.snapshot(),
      ...count !== void 0 ? { restorePoints: count } : {},
      ...identity
    };
  }
