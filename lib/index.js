...
var MemoryManager = class _MemoryManager {
  ...
  pendingPath(cwd) {
    return join2(cwd, MEMORY_DIR_NAME, PENDING_FILE_NAME);
  }
  async persistPending(cwd) {
    try {
      const forCwd = this.pending.filter((write) => write.cwd === cwd);
      await mkdir2(join2(cwd, MEMORY_DIR_NAME), { recursive: true });
...
function freshTempIndexPath() {
  for (let attempt = 0; attempt < 5; attempt++) {
    const candidate = join3(tmpdir(), `dsh-omp-advisor-index-${randomBytes(8).toString("hex")}`);
  ...
  return join3(tmpdir(), `dsh-omp-advisor-index-${randomBytes(12).toString("hex")}-${Date.now()}`);
}
...
import { resolve, relative, join as join4 } from "node:path";
...
async function loadContextFiles(cwd) {
  ...
  for (const name2 of CONTEXT_FILE_NAMES) {
    try {
      const content = await readFile4(join5(cwd, name2), "utf8");
